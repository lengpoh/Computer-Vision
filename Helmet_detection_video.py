...
# Draw a bounding box.
    ...
    if classes:
        print(classId, len(classes), label)
    ...
    if label_name == 'helmet':
        ...
        cv.rectangle(frame, (left, top - round(1.5*labelSize[1])), (left + round(1.0*labelSize[0]), top + baseLine), (255, 255, 255), cv.FILLED)
        cv.putText(frame, label_name, (left, top), cv.FONT_HERSHEY_SIMPLEX, 0.50, (0,0,0), 1)
        frame_count+=1

    if label_name == 'no_helmet':
        cv.rectangle(frame, (left, top), (right, bottom), (255, 153, 255), 3)
        #will try to print of label have people.. or can put a counter to find the no of people occurance.
        #will try if it satisfy the condition otherwise, we won't print the boxes or leave it.
        cv.rectangle(frame, (left, top - round(1.0*labelSize[1])), (left + round(1.0*labelSize[0]), top + baseLine), (255, 255, 255), cv.FILLED)
        cv.putText(frame, label_name, (left, top), cv.FONT_HERSHEY_SIMPLEX, 0.55, (0,1,1), 1)
        frame_count+=1
...
outv = cv.VideoWriter('outpy.avi',cv.VideoWriter_fourcc('M','J','P','G'), 10, (frame_width,frame_height))
